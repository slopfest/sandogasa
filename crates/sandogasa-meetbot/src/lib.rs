...
/// The action items recorded in a meeting's text minutes, under
/// "Action items": one `* text` line each, up to the first blank line
/// (an empty section yields none).
pub fn parse_action_items(minutes: &str) -> Vec<String> {
    let mut lines = minutes.lines();
    lines.find(|l| l.trim() == "Action items");
    // Skip the underline only: an empty section is blank lines, and a
    // blank line also "consists of dashes" to `all`.
    lines
        .skip_while(|l| !l.trim().is_empty() && l.trim().chars().all(|c| c == '-'))
        .take_while(|l| !l.trim().is_empty())
        .filter_map(|l| l.trim().strip_prefix("* ").map(|t| t.trim().to_string()))
        .collect()
}
...
impl Meetbot {
    ...
    /// The text minutes of `meeting` (the `.txt` twin of its summary).
    pub fn minutes(&self, meeting: &Meeting) -> Result<String, Box<dyn std::error::Error>> {
    ...
    pub fn attendees(
        ...
        Ok(parse_attendees(&self.minutes(meeting)?))
    }
}
...
Generated by MeetBot\n";
    ...
    #[test]
    fn action_items_come_from_their_block_and_may_be_empty() {
        let minutes = "Meeting ended at 2026-08-26 16:05:05\n\n\
                       Action items\n------------\n\
                       * Davide Cavalca and Neal Gompa to submit the report \n\
                       * Michel to check which slot is free\n\n\
                       People Present (lines said)\n";
        assert_eq!(
            parse_action_items(minutes),
            [
                "Davide Cavalca and Neal Gompa to submit the report",
                "Michel to check which slot is free"
            ]
        );
        let none = "Action items\n------------\n\n\nPeople Present (lines said)\n* @a:b (1)\n";
