...
- **Sweep the GitHub issue tracker before cutting a release, and again when planning the next stretch of work.** `gh issue list --state open`, then check each against the code, not against memory: an issue is often fixed in passing by work that never mentioned it (#3 was closed by a check written three months earlier; #11 by a rewrite that retired the flow it described). Close the stale ones with the fixing commit's hash and subject and the first release tag that shipped it — `git log --diff-filter=A -- <file>` or `git log -S<term>` finds the commit, `git tag --contains <sha> -l 'v*' | sort -V | head -1` the tag; if the commit cannot be pinned down, the release alone is fine. Sort what remains into before-release and after, and say which in the issue. Corollary that makes the sweep cheap: **when a commit implements or obsoletes a tracked issue, close the issue in the same sitting**, with the same commit-and-release pointer, rather than leaving it for the sweep to find
...
- Each tool must support `--version` and display its name, version, and short description (matching `Cargo.toml` `description`) in the `--help` header. In clap, use `#[command(version, long_version = sandogasa_cli::version!(), about, long_about = None, max_term_width = 80, before_help = sandogasa_cli::banner!())]`, with a three-line `build.rs` calling `sandogasa_build::emit_git_describe()` and `sandogasa-build = { workspace = true }` under `[build-dependencies]`. `version!()` is the `git describe --tags --dirty --always` of the checkout when built from one (`v0.22.0-74-ge11cff1-dirty`) and the crate version otherwise, so `--version` and the `--help` header say which checkout a dev binary is; `-V` and the man pages keep the plain crate version (`sandogasa_cli::man` strips the long version), so pages do not churn per commit and tarball builds stay reproducible. The build script must never fail: no git, or no checkout, means no describe. The `max_term_width = 80` is what enforces the 80-column help rule — it needs clap's `wrap_help` cargo feature (set in the workspace `Cargo.toml`), without which clap compiles out all wrapping and the attribute is silently inert. Clap never wraps the `Usage:` line itself, so a subcommand with several long positional value names can still exceed 80 — keep value names short (`PATH`, not `OUTPUT_FILE_PATH`)
...
- Run `cargo cov` at stability points — before release tagging, and when catching a feature up for its README/CHANGELOG entry. Coverage must stay at or above 81.5% line coverage at those gates (raised from 80% at v0.24.0, which shipped at 81.97%; ratchet it up as coverage grows) (binary `src/main.rs` files are excluded from the measurement; see `.cargo/config.toml`)
