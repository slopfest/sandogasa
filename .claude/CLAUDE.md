...
- **Never hand-edit `.github/workflows/release.yml`** — it is generated from `[workspace.metadata.dist]` in the root `Cargo.toml` by cargo-dist, the way the man pages are generated from clap. After changing any dist config, run `dist generate` and commit the result; `dist plan` shows what a release would produce and `dist build --artifacts=local --target=<triple>` builds one for real
