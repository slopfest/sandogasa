...
Installing dbranch meant having a Rust toolchain and building it, which
is a poor fit for the one tool here aimed at Debian and Ubuntu — where
sandogasa is not packaged at all, so there is no distribution route
either. Each release now carries statically-linked binaries for x86_64
and aarch64 Linux, and `cargo binstall dbranch` fetches them.
