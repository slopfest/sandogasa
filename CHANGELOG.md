...
file, as mechanical as a rust2rpm one — still had to be written out by
hand from the full fedora-review template, because the tool rejected
anything not generated by rust2rpm. The Encapsule Change for F45 was
waiting on several such reviews.

The digest now recognizes `# generated by cabal-rpm` and runs the same
checklist with Haskell evidence: the packaged version against the
highest non-deprecated version on Hackage, the spec `License:` against
the `.cabal` `license:` (a legacy Cabal identifier such as `BSD3` or
`GPL-3` counts as equivalent to the SPDX expression cabal-rpm writes for
it), tests enabled by `%bcond tests`, and a package whose `.cabal`
declares no test suite has its caveat pre-filled with that reason.
fedora-review's demand for a `gcc`/`gcc-c++`/`clang` BuildRequires is
dropped as benign, since GHC drives gcc itself and `ghc-compiler`
requires it. The `.cabal` is read out of the source tarball with
`tar`, since the built RPMs do not ship it. The post-import block names the release-monitoring
Hackage backend and `@haskell-lang-sig`. A cabal-rpm `Name:
ghc-%{pkg_name}` is expanded so the package reads as itself.
...
### dbranch ships prebuilt binaries for `cargo binstall`

Installing dbranch meant compiling it, which is a poor fit for the one
tool here aimed at Debian and Ubuntu — where sandogasa is not packaged
at all, so there is no distribution route either. Each release now
carries statically-linked binaries for x86_64 and aarch64 Linux, and
`cargo binstall dbranch` fetches them instead of building. That still
needs cargo, `binstall` being a cargo subcommand; someone with no Rust
at all can take the archive off the releases page directly.

The build runs on GitHub Actions, generated by
[cargo-dist](https://github.com/axodotdev/cargo-dist) from
`[workspace.metadata.dist]` in the root `Cargo.toml`. Both architectures
build natively — arm64 hosted runners have been free for public
repositories since August 2025, so nothing is cross-compiled or
emulated. Archives are named `dbranch-<target>.tar.xz`, which
cargo-binstall discovers without any `[package.metadata.binstall]`
configuration.

Only dbranch ships this way. The workspace sets `dist = false` and a
tool opts in with `[package.metadata.dist] dist = true`, so adding
another is one line — but the other fifteen drive Fedora and CentOS
services and reach their audience through packaging instead.

musl rather than glibc because a binary linked against the runner
image's glibc 2.39 will not start on EL9, and the point of shipping
binaries is to reach people who are not building from source. Nothing
in the workspace links OpenSSL — reqwest is configured for rustls
throughout — so a static build costs nothing.

`precise-builds = true`, so cargo-dist builds dbranch alone rather than
its default `cargo build --workspace`. That is not a speed choice: a
workspace build unifies features across all sixteen tools, which turns
sandogasa-cli's `tls` feature back on and links ring into dbranch after
all. ring compiles C and assembly, and on a musl target cc-rs insists on
a compiler named `<arch>-linux-musl-gcc` while Debian and Ubuntu's
`musl-tools` installs only `musl-gcc` — and cc-rs does not fall back to
it, so a build with `musl-gcc` on `PATH` still fails with `ToolNotFound`.
Building the one app that ships keeps its tree pure Rust and the runner
free of a C toolchain entirely.

`create-release = false`: the workflow uploads into a draft release that
already exists rather than generating a title and body from this file,
which keeps the hand-written tag message on the release page. The
release process in `.claude/CLAUDE.md` records the ordering that needs.
...
the Fedora Data WG wants to work with — and with three forms in play, both
subcommands take one selector for them: `--format text,json,csv`, comma
separated or repeated.

```sh
koji-lag report  --store lag.sqlite --since 2026-07-03 --out day/ --format json
koji-lag reports --store lag.sqlite --reports-root reports --format text,json,csv
```

Writing to a directory still defaults to `text,json`, so existing
invocations produce exactly what they did. What was not expressible before
is: only JSON, or only the CSVs. Printing to stdout takes one form, since a
stream is one file — `--json` remains the shorthand every tool here
accepts, and `--format csv` is refused with the reason rather than printing
one of its tables.

CSV is a file per table — `all-builds.csv`, `srpm-rebuild.csv`,
`multi-arch.csv`, `single-arch.csv`, `noarch-by-host.csv`, plus
`official.csv` and `scratch.csv` where the split applies — because a CSV
holds one table where the other two carry every table for a period
together. A period counts as already reported when the forms asked for are
present, so adding `csv` to a tree that has text and JSON writes the CSVs
instead of skipping the period as done.
