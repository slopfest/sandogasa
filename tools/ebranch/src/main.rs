...
struct ResolveArgs {
    ...
    #[arg(
        ...
        long_help = "\
Generate a shell script for Copr batch builds
(check-crate's --copr names the staging COPR a
check reads from).
...
struct CheckCrateArgs {
    ...
    /// Default: the config file's `[check-crate.copr]` entry for the
    /// crate. (The build-script generator is --copr-script.)
...
fn main() -> ExitCode {
    ...
    if let Command::CheckCrate(a) = &cli.command {
        ...
        let staging_copr = a
            .copr
            .clone()
            .or_else(|| config::check_crate_copr(a.name.as_deref().unwrap_or_default()));
        if let Some(c) = &staging_copr
            && check_update::parse_copr_spec(c).is_none()
        {
            return exit_code(Err(format!(
                "--copr wants owner/project (e.g. @rust/uutils-and-nushell), got {c}"
            )));
        }
...
fn print_copr_script(phases: &[dag::BuildPhase], reason: &dyn Fn(&str) -> Option<String>) {
    println!(
        r#"#!/bin/bash
# Generated by ebranch --copr-script
...
mod tests {
    ...
    fn man_page_matches_cli() {
        sandogasa_cli::man::check::<super::Cli>(
            concat!(env!("CARGO_MANIFEST_DIR"), "/man/ebranch.1"),
