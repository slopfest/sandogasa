...
enum Command {
    ...
    Config,
    /// Inventory the runtime dependencies pulled from other
    /// repos (e.g. EPEL).
    Deps(DepsArgs),
...
fn cmd_deps(paths: &[String], args: &DepsArgs) -> CmdResult {
    ...
    let name = args
        .name
        .clone()
        .unwrap_or_else(|| format!("{}-deps-{}", inventory.inventory.name, args.branch));
    let written = if let Some(path) = &args.output {
        let description = format!(
            "Runtime dependencies of inventory '{}' from repos [{}], \
