...
/// Render the actionable verdicts as a shell script: every command
/// echoed and individually confirmed, so the act phase is a
/// deliberate walk rather than one irreversible paste. The report
/// stays human-only; this is where the commands live.
pub fn format_script(report: &KondoReport, user: &str) -> String {
    let mut orphan: Vec<&str> = Vec::new();
    let mut self_remove: Vec<&str> = Vec::new();
    for c in &report.culled {
        match c.action {
            Action::Orphan => orphan.push(&c.name),
            Action::SelfRemove => self_remove.push(&c.name),
            _ => {}
        }
    }
    let mut out = format!(
        concat!(
            "#!/bin/bash\n",
            "# Cull actions for {user}: {orphans} orphanable, ",
            "{removes} self-removable.\n",
            "# Generated by poi-tracker announce; each command is echoed\n",
        ...
        user = user,
        orphans = orphan.len(),
        removes = self_remove.len(),
    );
    if !orphan.is_empty() {
        out.push_str("\n# owner — orphan in one batch:\n");
        out.push_str(&format!(
            "run sandogasa-pkg-acl give orphan {}\n",
            orphan.join(" ")
        ));
    }
    if !self_remove.is_empty() {
        out.push_str("\n# admin — remove own ACL, one package at a time:\n");
        for name in &self_remove {
            out.push_str(&format!(
                "run sandogasa-pkg-acl remove {name} --user {user}\n"
            ));
        }
    }
    out
...
mod tests {
    ...
    #[test]
    fn the_script_confirms_each_command_and_the_report_carries_none() {
        let report = KondoReport {
            culled: vec![
                Culled {
                    name: "old-toy".into(),
        ...
        let script = format_script(&report, "me");
        assert!(script.starts_with("#!/bin/bash\n"));
        assert!(script.contains("run sandogasa-pkg-acl give orphan old-toy gone-lib\n"));
        assert!(script.contains("run sandogasa-pkg-acl remove helped-once --user me\n"));
        assert!(script.contains("read -r -p \"run? [y/N] \""));
