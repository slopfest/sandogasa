...
A spec produced by a generator — `rust2rpm` for Rust crates, `cabal-rpm`
for Haskell packages (and, later, `pyp2spec` for Python packages) — is
mechanically correct by
...
- `curl` — for the crates.io / Hackage latest-version check; skip it
  with `--no-net`.
- `tar` — to read the `.cabal` file out of a Haskell package's source
  tarball (the built RPMs do not ship it).
...
- `-y`/`--yes` accepts every inferred mark without prompting (and skips
  ...
  a terminal.
- `--no-net` skips the crates.io / Hackage check.
...
- **Haskell (`cabal-rpm`)** — the same list with Hackage and the
  `.cabal` file as the evidence: the version is compared with the
  highest non-deprecated version on **Hackage**; the license with the
  `.cabal` `license:`, where a legacy Cabal identifier (`BSD3`,
  `GPL-3`) counts as equivalent to the SPDX expression cabal-rpm
  writes for it (`BSD-3-Clause`, `GPL-3.0-or-later` — the legacy form
  cannot say `-only` or `-or-later`, so that suffix is not compared);
  tests are enabled by `%bcond tests 1`, and a package whose `.cabal`
  declares no `test-suite` gets its caveat pre-filled with "upstream
  ships no test suite" rather than "disabled — justify". The `.cabal` is read out of the tarball under
  `srpm-unpacked/` with `tar`. There is no statically-linked-deps
  item: Haskell libraries link statically by design and the guidelines
  do not fold their licenses. fedora-review's `CheckGccBuildRequires`
  failure ("no gcc, gcc-c++ or clang found in BuildRequires") is
  dropped as benign: GHC drives gcc as its assembler and linker and
  `ghc-compiler` requires it, so a cabal-rpm spec rightly names
  neither. The last item is "complies with Haskell
  Packaging Guidelines", and the post-import block names the
  release-monitoring `Hackage` backend and `@haskell-lang-sig`.
...
`rust2rpm` (Rust) and `cabal-rpm` (Haskell) today. `pyp2spec` (Python
packages from PyPI) reuses the same machinery with its own checklist and
boilerplate — planned. The generator is detected from the spec's
`# Generated by …` marker; a hand-written spec is rejected (there's
nothing to simplify).

## System-wide configuration

Settings are read from `/etc/fedora-review-digest/config.toml` first,
then overridden per key by `~/.config/fedora-review-digest/config.toml`,
with command-line flags overriding both. A system file alone is enough —
no per-user file is required — and either may also carry a `[defaults]`
table pinning flag defaults (see the root `DEVELOPMENT.md`).

`fedora-review-digest config` writes the user file only, with 700 on the
