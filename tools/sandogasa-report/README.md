...
Settings are read from `/etc/sandogasa-report/config.toml` first, then
overridden per key by `~/.config/sandogasa-report/config.toml`, with
...
`sandogasa-report config` writes the user file only, with 700 on the
